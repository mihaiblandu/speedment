...
public abstract class GeneratedFilmEntityStoreSerializerImpl implements EntityStoreSerializer<Film> {
    ...
    private static final int FIELD_RENTAL_DURATION = 2;
    private static final int FIELD_FILM_ID = 4;
    private static final int FIELD_LAST_UPDATE = 8;
    private static final int FKREF_FK_FILM_LANGUAGE = 16;
    private static final int ENDPOS_ORIGINAL_LANGUAGE_ID = 24;
    private static final int ENDPOS_RELEASE_YEAR = 25;
    private static final int ENDPOS_LENGTH = 26;
    private static final int FKREF_ENDPOS_FK_FILM_LANGUAGE_ORIGINAL = 27;
    private static final int ENDPOS_TITLE = 28;
    private static final int ENDPOS_DESCRIPTION = 32;
    private static final int ENDPOS_RATING = 36;
    private static final int ENDPOS_SPECIAL_FEATURES = 40;
    private static final int ENDPOS_RENTAL_RATE = 44;
    private static final int ENDPOS_REPLACEMENT_COST = 48;
    private static final int VARSIZE_BEGINS = 52;
    ...
    public BiConsumer<ByteBuffer, Film> serializer() {
        return (buffer, entity) -> {
            ...
            if (entity.getReleaseYear().isPresent()) {
                buffer.putInt(VARSIZE_BEGINS + varSizePos, entity.getReleaseYear().getAsInt());
    ...
    public LongFunction<Film> deserializer() {
        return ref -> {
            ...
            if (buffer.get(offset + ENDPOS_RELEASE_YEAR) >= 0) {
                entity.setReleaseYear(buffer.getInt(offset + VARSIZE_BEGINS + buffer.get(offset + ENDPOS_RELEASE_YEAR) - Integer.BYTES));
            }
        ...
        if (colId instanceof Film.Identifier) {
            ...
            switch (_id) {
                ...
                case RELEASE_YEAR : return ref -> {
                    final ByteBuffer buffer = bufferFinder.apply(ref);
                    final int offset = offsetFinder.applyAsInt(ref);
                    return buffer.getInt(offset + VARSIZE_BEGINS + buffer.get(offset + ENDPOS_RELEASE_YEAR) - Integer.BYTES);
                };
            ...
            switch (_id) {
                ...
                case RELEASE_YEAR         : return (aRef, bRef) -> {
                    ...
                    return Integer.compare(
                        aBuf.getInt(aOffset + VARSIZE_BEGINS + aBuf.get(aOffset + ENDPOS_RELEASE_YEAR) - Integer.BYTES),
                        bBuf.getInt(bOffset + VARSIZE_BEGINS + bBuf.get(bOffset + ENDPOS_RELEASE_YEAR) - Integer.BYTES)
                    );
                };
