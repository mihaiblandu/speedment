...
public abstract class GeneratedCustomerEntityStoreSerializerImpl implements EntityStoreSerializer<Customer> {
    ...
    private static final int BITMAP_ACTIVE = 0;
    private static final int FIELD_STORE_ID = 1;
    private static final int FIELD_CUSTOMER_ID = 3;
    private static final int FIELD_ADDRESS_ID = 7;
    private static final int FIELD_CREATE_DATE = 11;
    private static final int FKREF_FK_CUSTOMER_ADDRESS = 19;
    private static final int FKREF_FK_CUSTOMER_STORE = 27;
    private static final int ENDPOS_LAST_UPDATE = 35;
    private static final int ENDPOS_FIRST_NAME = 36;
    private static final int ENDPOS_LAST_NAME = 40;
    private static final int ENDPOS_EMAIL = 44;
    private static final int VARSIZE_BEGINS = 48;
    ...
    public BiConsumer<ByteBuffer, Customer> serializer() {
        return (buffer, entity) -> {
            ...
            if (entity.getLastUpdate().isPresent()) {
                buffer.putLong(VARSIZE_BEGINS + varSizePos, entity.getLastUpdate().get().getTime());
                varSizePos += Long.BYTES;
                buffer.put(ENDPOS_LAST_UPDATE, (byte) varSizePos);
            } else {
                buffer.put(ENDPOS_LAST_UPDATE, (byte) (0x80 | varSizePos));
            }
        ...
        return ref -> {
            ...
            try {
                entity.setFirstName(Utf8Util.deserialize(buffer,
                    offset + VARSIZE_BEGINS + (0x7f & buffer.get(offset + ENDPOS_FIRST_NAME - Byte.BYTES)),
            ...
            entity.setActive((buffer.get(offset + BITMAP_ACTIVE) & 0b1) != 0);
            entity.setCreateDate(new Timestamp(buffer.getLong(offset + FIELD_CREATE_DATE)));
            if (buffer.get(offset + ENDPOS_LAST_UPDATE) >= 0) {
                entity.setLastUpdate(new Timestamp(buffer.getLong(offset + VARSIZE_BEGINS + buffer.get(offset + ENDPOS_LAST_UPDATE) - Long.BYTES)));
            }
            return entity;
        };
    }
    ...
    public LongPredicate isNull(final ColumnIdentifier<Customer> colId) {
        if (colId instanceof Customer.Identifier) {
            ...
            switch (_id) {
                ...
                case CUSTOMER_ID : 
                case ADDRESS_ID  : 
                case CREATE_DATE : return ALWAYS_FALSE;
                case LAST_UPDATE : return ref -> bufferFinder.apply(ref).get(offsetFinder.applyAsInt(ref) + ENDPOS_LAST_UPDATE) < 0;
                case FIRST_NAME  : 
    ...
    public LongPredicate isPresent(final ColumnIdentifier<Customer> colId) {
        if (colId instanceof Customer.Identifier) {
            ...
            switch (_id) {
                ...
                case CUSTOMER_ID : 
                case ADDRESS_ID  : 
                case CREATE_DATE : return ALWAYS_TRUE;
                case LAST_UPDATE : return ref -> bufferFinder.apply(ref).get(offsetFinder.applyAsInt(ref) + ENDPOS_LAST_UPDATE) >= 0;
                case FIRST_NAME  : 
    ...
    public LongToBooleanFunction booleanDeserializer(final ColumnIdentifier<Customer> colId) {
        if ("active".equals(colId.getColumnId())) {
            return ref -> (bufferFinder.apply(ref).get(offsetFinder.applyAsInt(ref) + BITMAP_ACTIVE) & 0b1) != 0;
        }
        ...
        if (colId instanceof Customer.Identifier) {
            ...
            switch (_id) {
                ...
                case LAST_UPDATE : return ref -> {
                    final ByteBuffer buffer = bufferFinder.apply(ref);
                    final int offset = offsetFinder.applyAsInt(ref);
                    return new Timestamp(buffer.getLong(offset + VARSIZE_BEGINS + buffer.get(offset + ENDPOS_LAST_UPDATE) - Long.BYTES));
                };
        ...
        if (colId instanceof Customer.Identifier) {
            ...
            switch (_id) {
                case ACTIVE      : return (aRef, bRef) -> Integer.compare(
                    bufferFinder.apply(aRef).get(offsetFinder.applyAsInt(aRef) + BITMAP_ACTIVE) & 0b1,
                    bufferFinder.apply(bRef).get(offsetFinder.applyAsInt(bRef) + BITMAP_ACTIVE) & 0b1
                );
                ...
                case LAST_UPDATE : return (aRef, bRef) -> {
                    ...
                    return Long.compare(
                        aBuf.getLong(aOffset + VARSIZE_BEGINS + aBuf.get(aOffset + ENDPOS_LAST_UPDATE) - Long.BYTES),
                        bBuf.getLong(bOffset + VARSIZE_BEGINS + bBuf.get(bOffset + ENDPOS_LAST_UPDATE) - Long.BYTES)
                    );
                };
                case FIRST_NAME  : return (aRef, bRef) -> {
                    ...
                    final int aOffset = offsetFinder.applyAsInt(aRef);
                    final int bOffset = offsetFinder.applyAsInt(bRef);
                    final int aStarts = aOffset + VARSIZE_BEGINS + (0x7f & aBuf.get(aOffset + ENDPOS_FIRST_NAME - Byte.BYTES));
                    final int bStarts = bOffset + VARSIZE_BEGINS + (0x7f & bBuf.get(bOffset + ENDPOS_FIRST_NAME - Byte.BYTES));
    ...
    public BiLongToIntFunction comparatorNullsLast(final ColumnIdentifier<Customer> colId) {
        if (colId instanceof Customer.Identifier) {
            ...
            switch (_id) {
                case ACTIVE      : return (aRef, bRef) -> Integer.compare(
                    bufferFinder.apply(aRef).get(offsetFinder.applyAsInt(aRef) + BITMAP_ACTIVE) & 0b11,
                    bufferFinder.apply(bRef).get(offsetFinder.applyAsInt(bRef) + BITMAP_ACTIVE) & 0b11
                );
                case STORE_ID    : return (aRef, bRef) -> Short.compare(
                    bufferFinder.apply(aRef).getShort(offsetFinder.applyAsInt(aRef) + FIELD_STORE_ID),
                    bufferFinder.apply(bRef).getShort(offsetFinder.applyAsInt(bRef) + FIELD_STORE_ID)
                ...
                case LAST_UPDATE : return (aRef, bRef) -> {
                    ...
                    final int aOffset = offsetFinder.applyAsInt(aRef);
                    final int bOffset = offsetFinder.applyAsInt(bRef);
                    final int aEndPos = aBuf.get(aOffset + ENDPOS_LAST_UPDATE);
                    final int bEndPos = bBuf.get(bOffset + ENDPOS_LAST_UPDATE);
    ...
    public LongToIntFunction compareToBoolean(final ColumnIdentifier<Customer> colId, final boolean value) {
        if ("active".equals(colId.getColumnId())) {
            return value
                ? ref -> Integer.compare(bufferFinder.apply(ref).get(offsetFinder.applyAsInt(ref) + BITMAP_ACTIVE) & 0b1, 0b1)
                : ref -> Integer.compare(bufferFinder.apply(ref).get(offsetFinder.applyAsInt(ref) + BITMAP_ACTIVE) & 0b1, 0);
            ...
            switch (_id) {
                ...
                case LAST_UPDATE : {
                    ...
                    return ref -> {
                        final ByteBuffer buffer = bufferFinder.apply(ref);
                        final int offset = offsetFinder.applyAsInt(ref);
                        final int endPos = buffer.get(offset + ENDPOS_LAST_UPDATE);
    ...
    public LongToIntFunction compareToNull(final ColumnIdentifier<Customer> colId) {
        if (colId instanceof Customer.Identifier) {
            final Customer.Identifier _id = (Customer.Identifier) colId;
            switch (_id) {
                case ACTIVE      : return ref -> -1;
                case STORE_ID    : 
                case CUSTOMER_ID : 
                case ADDRESS_ID  : 
                case CREATE_DATE : return ALWAYS_LESS;
                case LAST_UPDATE : return ref -> {
                    final ByteBuffer buffer = bufferFinder.apply(ref);
                    final int offset = offsetFinder.applyAsInt(ref);
                    return buffer.get(offset + ENDPOS_LAST_UPDATE) < 0 ? 0 : -1;
                };
    ...
    public LongPredicate newPredicate(final ColumnIdentifier<Customer> colId, final EntityStorePredicate predicate) {
        if (colId instanceof Customer.Identifier) {
            final Customer.Identifier _id = (Customer.Identifier) colId;
            switch (_id) {
                case ACTIVE      : return ref -> {throw new UnsupportedOperationException("No data stored for column " + colId.getColumnId() + ".");};
                ...
                case LAST_UPDATE : return ref -> {
                    ...
                    return predicate.test(buffer,
                        ...
                        rowOffset + VARSIZE_BEGINS + buffer.get(rowOffset + ENDPOS_LAST_UPDATE)
                    );
                };
                case FIRST_NAME  : return ref -> {
                    final ByteBuffer buffer = bufferFinder.apply(ref);
                    final int rowOffset = offsetFinder.applyAsInt(ref);
                    return predicate.test(buffer,
                        rowOffset + VARSIZE_BEGINS + (0x7f & buffer.get(rowOffset + ENDPOS_FIRST_NAME - Byte.BYTES)),
    ...
    public BiLongToIntFunction newComparator(final ColumnIdentifier<Customer> colId, final EntityStoreComparator comparator) {
        if (colId instanceof Customer.Identifier) {
            final Customer.Identifier _id = (Customer.Identifier) colId;
            switch (_id) {
                case ACTIVE      : return (aRef, bRef) -> {throw new UnsupportedOperationException("No data stored for column " + colId.getColumnId() + ".");};
                ...
                case LAST_UPDATE : return (aRef, bRef) -> {
                    ...
                    final int bRowOffset = offsetFinder.applyAsInt(bRef);
                    return comparator.compare(
                        aBuffer, aRowOffset + VARSIZE_BEGINS, aRowOffset + VARSIZE_BEGINS + aBuffer.get(aRowOffset + ENDPOS_LAST_UPDATE), 
                        bBuffer, bRowOffset + VARSIZE_BEGINS, bRowOffset + VARSIZE_BEGINS + bBuffer.get(bRowOffset + ENDPOS_LAST_UPDATE)
                    );
                };
                case FIRST_NAME  : return (aRef, bRef) -> {
                    final ByteBuffer aBuffer = bufferFinder.apply(aRef);
                    ...
                    final int bRowOffset = offsetFinder.applyAsInt(bRef);
                    return comparator.compare(
                        aBuffer, aRowOffset + VARSIZE_BEGINS + (0x7f & aBuffer.get(aRowOffset + ENDPOS_FIRST_NAME - Byte.BYTES)), aRowOffset + VARSIZE_BEGINS + aBuffer.getInt(aRowOffset + ENDPOS_FIRST_NAME), 
                        bBuffer, bRowOffset + VARSIZE_BEGINS + (0x7f & bBuffer.get(bRowOffset + ENDPOS_FIRST_NAME - Byte.BYTES)), bRowOffset + VARSIZE_BEGINS + bBuffer.getInt(bRowOffset + ENDPOS_FIRST_NAME)
                    );
                };
                case LAST_NAME   : return (aRef, bRef) -> {
                    final ByteBuffer aBuffer = bufferFinder.apply(aRef);
